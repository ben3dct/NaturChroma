...
BOOL My_Chroma_Implementation::example_keyboard() {
	...
	if (WdataT >= 30)
	{
		ChromaSDK::Keyboard::STATIC_EFFECT_TYPE Keyboard_Condition_Extreme = {}; //Initialize
		Keyboard_Condition_Extreme.Color = RED;
	...
	else if (WdataT > 5 && WdataT < 15)
	{
		...
		RZRESULT Result_Keyboard = CreateKeyboardEffect(ChromaSDK::Keyboard::CHROMA_STATIC, &Keyboard_Condition_IExtreme, nullptr); // applied static lighting to keyboard
		return Result_Keyboard;
	}
	else
	{
		ChromaSDK::Keyboard::STATIC_EFFECT_TYPE Keyboard_Condition_DEATH = {}; //Initialize
		Keyboard_Condition_DEATH.Color = WHITE;
		RZRESULT Result_Keyboard = CreateKeyboardEffect(ChromaSDK::Keyboard::CHROMA_STATIC, &Keyboard_Condition_DEATH, nullptr); // applied static lighting to keyboard
		return Result_Keyboard;
	}
